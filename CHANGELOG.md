...
### Added
- **Google Gemini support**: Added support for Google Gemini AI models
  - New provider type `gemini` in `PEEKABOO_AI_PROVIDERS` configuration
  - Requires `GEMINI_API_KEY` environment variable
  - Default model: `gemini-2.5-flash`
  - Provider status checking (API key presence only, no validation)
